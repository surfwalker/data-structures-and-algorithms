...
const countNumberOfChildren = (arr) => {
  return arr.reduce(((accumulator, element) => accumulator + ((element.children) ? element.children.length : 0)), 0);
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((accumulator, element) => isPrime(element) ? accumulator + 1: accumulator, 0);
};
...
const extractChildren = (arr) => {
  let namesWithA = arr.filter(element => element.name.includes('a'));
  return namesWithA.reduce((accumulator, element) => {
    if(element.children) {
      for (let index in element.children){
        accumulator.push(element.children[index]);
      }
    }
    return accumulator;
  }, []);
