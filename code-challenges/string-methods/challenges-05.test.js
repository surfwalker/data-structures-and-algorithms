...
const listFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(element => result.push(element.slice((element.indexOf(' ', 3)) + 1)));
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(element => result.push(element.split(' ').splice(2).join(' ')));
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  recipe.steps.forEach(element => result.push(element.split(' ').shift()));
  return result;
};
...
// I would really like this function to work but I can't get it to print the space at the end as I can with the for loop
...
// doing the same as the above .forEach() but with a for loop to get the space to print after the arr.length has been reached
const removeEvenValues = (arr) => {
...
const removeLastCharacters = (str, numberOfCharacters) => {
  return numberOfCharacters < 0 ? str : numberOfCharacters > str.length ? '' : str.slice(0, -numberOfCharacters);
};
...
Write a function named totalSumCSV that, given a string of comma-separated values (CSV) as input. (e.g.  ), returns the total sum of the numeric values (e.g. 6).
------------------------------------------------------------------------------------------------ */
...
const removeVowels = (str) => {
  ...
  str.split('').forEach((element) => {
    if((!(element === 'a'||element === 'e'||element === 'i'||element === 'o'||element === 'u'||element === ' ') && (element.match(/[a-z]/i)))) {
      newStr.push(element);
    }
  });
  return newStr.join('');
...
const extractVowels = (str) => {
  ...
  return [str.join(''), vowels.sort().join('')];
};
