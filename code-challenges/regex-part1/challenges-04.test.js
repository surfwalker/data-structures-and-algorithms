...
// passes all test except for one expecting empty array when instead it returns "null"
...
const citiesAtoJ = (arr) => {
  const citiesAToJ = [];
  arr.forEach(element => {
    if((/(^[A-J])/g).test(element)) citiesAToJ.push(element);
  });
  return citiesAToJ;
};
