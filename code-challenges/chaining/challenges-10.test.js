...
const count = (target, input) => {
  return input.map(element => element.filter(int => int === target)).reduce((accumulator, element) => accumulator + element.length, 0);
};
...
const totalSum = (input) => {
  return input.map(element => element.reduce((accumulator, element) => accumulator + element), 0).reduce((accumulator, element) => accumulator + element, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(element => element.filter(element => Number.isInteger(element) && !(element % 5)).map(element => Math.pow(2, element)));
};
